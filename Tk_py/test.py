...
class HandGestureCameraApp(QMainWindow):
    def __init__(self):
        ...
        # ----------------- SignTra label at top-left -----------------
        self.sign_label = QLabel("SignTra")
        self.sign_label.setFont(QFont("Arial", 28, QFont.Bold))
        self.sign_label.setStyleSheet("color: white;")  # change color if you want
        self.sign_label.setAlignment(Qt.AlignLeft | Qt.AlignTop)

        # Put the label in a small frame to manage margins
        sign_frame = QFrame()
        sign_frame.setFixedHeight(60)  # optional, gives space for the label
        sign_layout = QVBoxLayout(sign_frame)
        sign_layout.setContentsMargins(0, 0, 0, 0)
        sign_layout.addWidget(self.sign_label, alignment=Qt.AlignLeft | Qt.AlignVCenter)

        main_layout.addWidget(sign_frame)
